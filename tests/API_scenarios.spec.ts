...
Steps:
1. Send a GET request to the API endpoint that returns the list of books (https://demoqa.com/BookStore/v1/Books)
...
2. Send a POST request to the API endpoint (https://demoqa.com/BookStore/v1/Books) with a payload including a books ISBN and user ID/token
...
test('Register user, generate token, and add a book to user collection', async ({ request }) => {
    ...
    const isbn = '9781449331818'; // Use a valid ISBN from the API's book collection
    const payload = {
        userId,
